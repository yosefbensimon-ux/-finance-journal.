                    ...
                    <div key={dIso} style={{ borderRight: i < 6 ? "1px solid #EEF0F2" : "none", display: "flex", flexDirection: "column" }}>
                      ...
                      <div style={{ flex: 1, padding: "0.3rem", display: "flex", flexDirection: "column", gap: "0.25rem", overflowY: "auto" }}>
                        ...
                        {dayEntries.map((e) => {
                          ...
                          return (
                            <div key={e.id} className="chip" title="לחצו לעריכה, קליק ימני לאפשרויות" onClick={() => openEditForm(e)} onContextMenu={(ev) => openContextMenu(ev, "entry", e)} style={{ background: entryStatusColor(e.status), borderRadius: "0.3rem", padding: "0.3rem 0.4rem", fontSize: "0.62rem", position: "relative", color: "#fff" }}>
                              <div style={{ position: "absolute", top: "3px", right: "3px", width: "7px", height: "7px", borderRadius: "50%", background: cat.color, border: "1px solid rgba(255,255,255,0.85)" }} title={cat.label} />
      ...
      {periodEntries.map((e) => {
        ...
        return (
          <div key={e.id} onClick={() => onEditEntry(e)} onContextMenu={(ev) => onContextMenuEntry(ev, e)} style={{ display: "flex", alignItems: "center", gap: "0.6rem", background: entryStatusColor(e.status), borderRadius: "0.5rem", padding: "0.6rem 0.8rem", cursor: "pointer", color: "#fff" }}>
            <span title={cat.label} style={{ width: "0.6rem", height: "0.6rem", borderRadius: "50%", background: cat.color, border: "1px solid rgba(255,255,255,0.85)", flexShrink: 0 }} />
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 600, fontSize: "0.85rem" }}>{e.partyName || cat.label}</div>
              <div style={{ fontSize: "0.72rem", opacity: 0.85 }}>{cat.label}{e.notes ? ` · ${e.notes}` : ""}</div>
            </div>
